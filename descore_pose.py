...
json_file_path = '/home/haoge/ymq1/frame_similarity_results.json'
keyframe_folder = '/home/haoge/ymq1/20160501_3_frames_output'
excel_file = '/home/haoge/ymq1/2016-3.xlsx'  # 需要写入得分的 Excel 文件路径
...
for video_file in video_files:
    ...
    if match:
        ...
        if last_end_frame is not None and (start_frame - last_end_frame) > 100:
            ...
            round_time.append(cumulative_time)  # 把得分添加到结果
            cumulative_time = 0
       ...
       # 遍历相关帧并累计得分
        for frame, similarity in relevant_frames.items():
            if frame not in processed_frames and similarity:  # 如果帧没有被处理过
                ...
                if last_frame_in_round is not None and (frame - last_frame_in_round) > 100:
                    ...
                    cumulative_score = 0  # 重置累计得分
                    cumulative_time+=(frame-start_frame)/25
                    round_time.append(cumulative_time)  # 把得分添加到结果
                    cumulative_time = -(frame-start_frame)/25

                # 计算得分
                x_value = similarity[0]  # 获取第二个数据作为x值
        ...
        cumulative_time += (last_frame_in_round-start_frame)/25
...
if cumulative_time > 0:
    round_time.append(cumulative_time)
# # 2016-1需要这个
...
# 保存新的Excel文件
df.to_excel('2016-3-pose-t.xlsx', index=False)
